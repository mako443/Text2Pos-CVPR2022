...
def create_poses_and_images(path_poses, path_images, path_out, db_dist=20, query_dist=5, step=4):
    ...
    for idx in range(0, len(poses), step):
        ...
        if np.min(dists) >= db_dist:
            ...
            copyfile(osp.join(path_images, f'{image_name:010.0f}.png'), osp.join(path_query, f'{len(sampled_query_poses) - 1:04.0f}.png'))
...
if __name__ == '__main__':
    ...
    path_images = osp.join('./data/kitti360-images/kitti360-data-2d', folder_name, 'image_00', 'data_rect')
