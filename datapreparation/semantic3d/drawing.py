            ...
            for do in pose_descriptions[i_p]:
                object3d = objects_dict[do.id]
                    ...
                    (
                        0.5
                        * (
                            np.max(object3d.points_w[:, 0:2], axis=0)
                            + np.min(object3d.points_w[:, 0:2], axis=0)
                        )
                ...
                cv2.arrowedLine(
                    img, (center[0], center[1]), (end[0], end[1]), (255, 255, 255), thickness=2
                )
    return img
...
def draw_objects_poses_viewObjects(objects, poses, view_objects, keys):
    ...
    for key in keys:
        ...
        for vo in vos:
            ...
            cv2.arrowedLine(
                img, (int(start[0]), int(start[1])), (int(end[0]), int(end[1])), (155, 155, 155), 3
            )
...
def draw_objects_poses_descriptions_DEPRECATED(objects, poses, descriptions):
    ...
    for pose, description in zip(poses, descriptions):
        for do in description:
            ...
            cv2.arrowedLine(
                img, (int(start[0]), int(start[1])), (int(end[0]), int(end[1])), (255, 255, 255), 3
            )
...
def draw_objects_objectDescription(objects, description_list):
    ...
    for d_obj in description_list:
        ...
        cv2.arrowedLine(
            ...
            (int(source_center[0]), int(source_center[1])),
            (128, 128, 0),
            thickness=3,
        )
...
def draw_cells(objects, cells, highlight_indices=[], poses=[], pose_descriptions=None):
    ...
    for idx, cell in enumerate(cells):
        ...
        cv2.rectangle(
            img, tuple(bbox[0:2]), tuple(bbox[2:4] - np.array((5, 5), dtype=np.int)), c, thickness=t
        )  # Subtract a few pixels to see different overlapping cells
...
def draw_retrieval(dataset, pose_idx, top_cell_indices, k=3):
    ...
    img = draw_objects_poses(
        dataset.scene_objects,
        dataset.poses[pose_idx : pose_idx + 1],
        draw_arrows=False,
        pose_descriptions=dataset.pose_descriptions[pose_idx : pose_idx + 1],
    )
    ...
    for idx in top_cell_indices[0:k]:
        ...
        cv2.rectangle(
            img,
            tuple(bbox[0:2] + offset[0:2]),
            tuple(bbox[2:4] + offset[2:4]),
            (0, 0, 255),
            thickness=2,
        )
