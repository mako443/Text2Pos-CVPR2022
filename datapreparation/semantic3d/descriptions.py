...
def select_view_objects(view_objects, pose, max_objects):
    view_objects = [
        ...
        if np.abs(vo.center[0] - 1 / 2 * IMAGE_WIDHT) <= 1 / 2 * IMAGE_WIDHT
    ]
    view_objects_sorted = sorted(
        view_objects, key=lambda vo: np.abs(vo.center[0] - 1 / 2 * IMAGE_WIDHT)
    )  # CARE: assumes [0] is always x-direction
...
def describe_pose_DEPRECATED(view_objects, poses, key, max_objects=3):
    ...
    for direction, key in direction_keys.items():
        ...
        description.extend(
            [DescriptionObject.from_view_object(vo, direction) for vo in selected_view_objects]
        )
...
def describe_object(scene_objects, idx, max_mentioned_objects=5, max_dist=25):
    ...
    description.append(
        DescriptionObject.from_object3d(target, None)
    )  # Direction=None means the object is the target
    ...
    for closest_index in closest_indices:
        ...
        if abs(diff[0]) >= abs(diff[1]) and diff[0] <= 0:
            direction = "west"
        if abs(diff[0]) <= abs(diff[1]) and diff[1] >= 0:
            direction = "north"
        if abs(diff[0]) <= abs(diff[1]) and diff[1] <= 0:
            direction = "south"
    ...
    for obj in scene_objects:
        ...
        if len(points_in_cell) / len(obj.points_w) >= min_fraction:
            ...
            cell_object = CellObject(
                obj.points_w,
                obj.points_w_color,
                points_in_cell_relative,
                points_in_cell_color,
                obj.label,
                obj.id,
                obj.color,
                obj.scene_name,
