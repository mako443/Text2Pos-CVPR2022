...
from datapreparation.semantic3d.drawing import (
    draw_objects_poses,
    draw_objects_poses_viewObjects,
    draw_viewobjects,
    draw_objects_objectDescription,
    combine_images,
)
from datapreparation.semantic3d.drawing import draw_cells, draw_objects_poseDescription
from datapreparation.semantic3d.descriptions import (
...
from semantic.imports import (
    ClusteredObject as ClusteredObject_S3D,
    ViewObject as ViewObject_S3D,
    COLORS,
    COLOR_NAMES,
)
...
def convert_s3d_data(path_pcd, path_images, split, scene_name):
    ...
    view_objects = pickle.load(
        open(osp.join(path_images, split, scene_name, "view_objects.pkl"), "rb")
    )
    view_objects = {
        k: [ViewObject.from_view_object_s3d(v, convert_color_s3d(v.color)) for v in vos]
        for k, vos in view_objects.items()
    }
...
def reduce_objects(objects, view_objects=None, count=16):
    ...
    for object_class in CLASSES_COLORS.keys():
        ...
        class_objects = sorted(
            class_objects,
            key=lambda o: np.abs(np.max(o.points_w[:, 0]) - np.min(o.points_w[:, 0]))
            * np.abs(np.max(o.points_w[:, 1]) - np.min(o.points_w[:, 1])),
            reverse=True,
        )
...
if __name__ == "__main__":
    ...
    if True:
        for scene_name in COMBINED_SCENE_NAMES:
            ...
            print(scene_name)
            objects, poses = convert_s3d_data_objectsPosesOnly(
                path_pcd, path_images, "train", scene_name
            )
            objects = [
                o
                for o in objects
                if o.label
                in ["high vegetation", "low vegetation", "buildings", "hard scape", "cars"]
            ]
            objects = reduce_objects(objects)

            poses = [
                p for p in poses if np.abs(p.phi) < 0.1
            ]  # Take poses only in one direction for now
            ...
            if not osp.isdir(output_dir_scene):
                os.mkdir(output_dir_scene)
            ...
            pickle.dump(
                cells, open(osp.join(output_dir_scene, "cell_object_descriptions.pkl"), "wb")
            )
            pickle.dump(
                pose_descriptions, open(osp.join(output_dir_scene, "pose_descriptions.pkl"), "wb")
            )
            print(
                f'Saved {len(objects)}, {len(cells)} cells with {mean_cell_objects:0.2f} avg. objects (cell-size {cell_size}) and {len(pose_descriptions)} pose-descriptions to {osp.join(output_dir,"train", scene_name)}'
            )
    ...
    objects = [
        o
        for o in objects
        if o.label in ["high vegetation", "low vegetation", "buildings", "hard scape", "cars"]
    ]
    ...
    pickle.dump(
        descriptions,
        open(osp.join(output_dir, "train", scene_name, "list_object_descriptions.pkl"), "wb"),
    )
    pickle.dump(
        texts, open(osp.join(output_dir, "train", scene_name, "text_object_descriptions.pkl"), "wb")
    )
    pickle.dump(
        hints, open(osp.join(output_dir, "train", scene_name, "hint_object_descriptions.pkl"), "wb")
    )
    pickle.dump(
        cells, open(osp.join(output_dir, "train", scene_name, "cell_object_descriptions.pkl"), "wb")
    )
    print(
        f'Saved {len(objects)} objects, {len(descriptions)} descriptions, {len(texts)} texts and {len(cells)} cells with {mean_cell_objects:0.2f} avg. objects to {osp.join(output_dir,"train", scene_name)}'
    )
    print()
    ...
    pickle.dump(
        out_descriptions,
        open(osp.join(output_dir, "train", scene_name, "pose_descriptions.pkl"), "wb"),
    )
    pickle.dump(
        out_descriptionTexts,
        open(osp.join(output_dir, "train", scene_name, "pose_description_texts.pkl"), "wb"),
    )
