...
data_set = TextDescriptionData(
    "data/semantic3d", "train", "sg27_station5_intensity_rgb", split_mask=train_mask
)
data_set_val = TextDescriptionData(
    "data/semantic3d", "train", "sg27_station5_intensity_rgb", split_mask=val_mask
)
data_loader = DataLoader(
    data_set, batch_size=BATCH_SIZE, num_workers=2, pin_memory=False, shuffle=True
)
...
for lr in (5e-3, 1e-3, 5e-4):
    ...
    )  # Adam is ok for PyG | Apparently also for packed_sequence!
    ...
    for epoch in range(32):
        ...
        for i_batch, batch in enumerate(data_loader):
            ...
            loss = criterion(out[:, 0:2], target[:, 0:2]) + BETA * criterion(
                out[:, 2:6], target[:, 2:6]
            )
