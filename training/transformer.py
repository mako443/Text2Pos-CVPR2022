...
def train_epoch(model, dataloader, args, do_print):
    ...
    return np.mean(epoch_losses), np.mean(epoch_acc_ref), np.mean(epoch_acc_tgtclass), np.mean(epoch_acc_objclass)
...
if __name__ == "__main__":
    ...
    for lr in learning_rates:
        model = TransformerMatch1( dataset_train.get_known_classes(), 
        ...
        criterion_ref_weights[0:data0['num_mentioned']] = data0['num_distractors'] / data0['num_mentioned']
        ...
        for epoch in range(args.epochs):
            ...
            loss, acc_ref, acc_tgtclass, acc_objclass = train_epoch(model, dataloader_train, args, do_print=(epoch==args.epochs-1))
            scheduler.step()
