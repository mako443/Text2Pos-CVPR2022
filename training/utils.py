...
def depr_plot_retrievals(top_retrievals, dataset, count=5):
    ...
    for query_idx in query_indices:
        ...
        if (
            query_idx in retrieval_indices[0:10]
        ):  # Only plot retrievals where the target is not in top-10
            continue
        ...
        print(
            f"Plotted {query_idx}: {retrieval_indices[0]} {retrieval_indices[1]} {retrieval_indices[2]}"
        )
