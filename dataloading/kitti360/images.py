...
class Kitti360ImageCompareDataset(Dataset):
    def __init__(self, base_path, scene_name, db_or_query, transform=None):
        ...
        self.image_paths = sorted(
            [osp.join(image_dirpath, f) for f in os.listdir(image_dirpath) if f.endswith(".png")]
        )
    ...
    def __getitem__(self, idx):
        ...
        return {"poses": self.poses[idx], "images": img}
...
class Kitti360ImageDataset(Dataset):
    def __init__(self, base_path, scene_name, split, transform=None):
        ...
        self.image_paths = np.array(
            sorted(
                [
                    osp.join(base_path, scene_name, "real", split, f)
                    for f in os.listdir(osp.join(base_path, scene_name, "real", split))
                    if f.endswith(".png")
                ]
            )
        )
        ...
        if osp.isdir(osp.join(base_path, scene_name, "rendered", split)):
            self.image_paths_rendered = np.array(
                sorted(
                    [
                        osp.join(base_path, scene_name, "rendered", split, f)
                        for f in os.listdir(osp.join(base_path, scene_name, "rendered", split))
                        if f.endswith(".png")
                    ]
                )
            )
        ...
        print(
            f"Kitti360ImageDataset ({self.scene_name}): {len(self)} poses, rendered: {self.image_paths_rendered is not None}"
        )
...
if __name__ == "__main__":
    dataset = Kitti360ImageCompareDataset(
        "./data/k360_30-10_scG_pd10_pc4_spY_all", "2013_05_28_drive_0010_sync", "db"
    )
    data = dataset[0]
