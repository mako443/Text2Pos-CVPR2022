...
class Semantic3dPosesDataset(Dataset):
    def __init__(self, path_numpy, path_scenes, scene_name, cell_size, cell_stride, split=None):
        ...
        if split is not None:
            ...
            self.pose_descriptions = [
                pd for (i, pd) in enumerate(self.pose_descriptions) if indices[i]
            ]

        # Create texts
        self.pose_texts = [
            self.create_pose_text(description) for description in self.pose_descriptions
        ]

        # Create cells
        self.cells, _ = create_cells(
            self.scene_objects, cell_size=self.cell_size, cell_stride=self.cell_stride
        )
        self.best_cell_indices = [self.find_best_cell(pose) for pose in self.poses]

        assert (
            len(self.poses)
            == len(self.pose_descriptions)
            == len(self.pose_texts)
            == len(self.best_cell_indices)
        )
    ...
    def plot(self, pose_indices="all"):
        if pose_indices == "all":
            pose_indices = np.arange(len(self))
...
class Semantic3dPosesDatasetMulti(Dataset):
    def __init__(self, path_numpy, path_scenes, scene_names, cell_size, cell_stride, split=None):
        ...
        self.split = split
        self.datasets = [
            Semantic3dPosesDataset(
                path_numpy, path_scenes, scene_name, cell_size, cell_stride, split=split
            )
...
if __name__ == "__main__":
    dataset = Semantic3dPosesDataset(
        "./data/numpy_merged/",
        "./data/semantic3d",
    ...
    dataset = Semantic3dPosesDatasetMulti(
        "./data/numpy_merged/",
        "./data/semantic3d",
        COMBINED_SCENE_NAMES,
        cell_size=65.0,
