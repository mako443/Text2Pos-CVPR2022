...
class Semantic3dObjectData(Dataset):
    def __init__(self, root, scene_name, description_length=12):
        ...
        self.pose_descriptions = pickle.load(
            open(osp.join(root, "train", scene_name, "descriptions.pkl"), "rb")
        )
        ...
        for obj in self.objects:
            self.object_positions.append(
                0.5 * (np.max(obj.points_w[:, 0:2], axis=0) + np.min(obj.points_w[:, 0:2], axis=0))
            )
    ...
    def plot_pose(self, idx):
        k = self.keys[idx]
        return draw_objects_poses_descriptions(
            self.objects, (self.poses[k],), (self.pose_descriptions[k],)
        )
    ...
    def __getitem__(self, index):
        ...
        if len(description) > self.description_length:
            description_objects = list(
                np.random.choice(description, size=self.description_length, replace=False)
            )
