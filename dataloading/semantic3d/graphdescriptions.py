...
class MockGraphData(Dataset):
    def __init__(self):
        ...
        self.object_edges = torch.tensor(
            [[0, 0, 0, 1, 1, 1, 2, 2, 2, 3, 3, 3], [1, 2, 3, 0, 2, 3, 0, 1, 3, 0, 1, 2]],
            dtype=torch.long,
        )

        self.description_edges = torch.tensor(
            [[0, 0, 0, 1, 1, 1, 2, 2, 2, 3, 3, 3], [1, 2, 3, 0, 2, 3, 0, 1, 3, 0, 1, 2]],
            dtype=torch.long,
        )
    ...
    def __getitem__(self, idx):
        ...
        with torch.no_grad():
            if idx == 0:
                description_nodes = self.description_embed(torch.arange(4) + 0)
            if idx == 1:
                description_nodes = self.description_embed(torch.arange(4) + 4)
            if idx == 2:
                description_nodes = self.description_embed(torch.arange(4) + 8)
            if idx == 3:
                description_nodes = self.description_embed(torch.arange(4) + 12)
        ...
        positive_edges = torch.stack(
            (positive_objects, torch.arange(4) + self.num_objects)
        )  # These edges are only needed 1-dimensional (dot-product is symmetric)
        ...
        return Data(
            x=nodes,
            edge_index=edges,
            edge_index_positive=positive_edges,
            edge_index_negative=negative_edges,
        )
...
def sample_negative_edges(positive_edges, num_objects, num_descriptions):
    ...
    for col in range(num_edges):
        negative_edges[0, col] = int(
            np.random.choice([i for i in range(num_objects) if i != positive_edges[0, col]])
        )
