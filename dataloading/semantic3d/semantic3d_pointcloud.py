...
class Semantic3dObjectDatasetMulti(Dataset):
    def __init__(
        ...
        scene_names,
        split=None,
        transform=T.Compose([T.FixedPoints(2048), T.NormalizeScale()]),
    ):
        ...
        self.datasets = [
            Semantic3dObjectDataset(
                path_numpy, path_scenes, scene_name, split=split, transform=transform
            )
...
class Semantic3dObjectDataset(Dataset):
    def __init__(
        ...
        if split is not None:
            ...
            self.scene_objects = [
                obj for (idx, obj) in enumerate(self.scene_objects) if indices[idx]
            ]
