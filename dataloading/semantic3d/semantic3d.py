...
from datapreparation.semantic3d.imports import (
    Object3D,
    DescriptionObject,
)  # , COMBINED_SCENE_NAMES, COLORS, COLOR_NAMES
...
class Semantic3dObjectReferenceMockDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(self.num_mentioned + self.num_distractors):
            ...
            objects.append(
                Object3D.from_mock_data(
                    {"points_w": points_w, "label": label, "color": color, "id": i}
                )
            )
        ...
        for i in range(self.num_mentioned):
            ...
            if i == 0:  # Target
                ...
                if abs(obj_to_target[0]) >= abs(obj_to_target[1]) and obj_to_target[0] >= 0:
                    direction = "east"
                if abs(obj_to_target[0]) >= abs(obj_to_target[1]) and obj_to_target[0] <= 0:
                    direction = "west"
                if abs(obj_to_target[0]) <= abs(obj_to_target[1]) and obj_to_target[1] >= 0:
                    direction = "north"
                if abs(obj_to_target[0]) <= abs(obj_to_target[1]) and obj_to_target[1] <= 0:
                    direction = "south"
        ...
        assert (
            len(all_matches) == self.num_mentioned + self.num_distractors
            and np.sum(all_matches[:, 1] == self.num_mentioned) == self.num_distractors
        )
...
class Semantic3dPoseReferenceMockDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        distances = np.linalg.norm(
            pose[0:2] - np.array([obj.center for obj in objects])[:, 0:2], axis=1
        )  # Distance only x&y
        ...
        for hint_idx, obj_idx in enumerate(sorted_indices[0 : self.num_mentioned]):
            ...
            if abs(obj_to_pose[0]) >= abs(obj_to_pose[1]) and obj_to_pose[0] >= 0:
                direction = "east"
            if abs(obj_to_pose[0]) >= abs(obj_to_pose[1]) and obj_to_pose[0] <= 0:
                direction = "west"
            if abs(obj_to_pose[0]) <= abs(obj_to_pose[1]) and obj_to_pose[1] >= 0:
                direction = "north"
            if abs(obj_to_pose[0]) <= abs(obj_to_pose[1]) and obj_to_pose[1] <= 0:
                direction = "south"
        ...
        for obj_idx in range(len(objects)):
            if obj_idx not in matches[:, 0]:  # If the object is not mentioned, i.e. in matches
                all_matches.append(
                    (obj_idx, self.num_mentioned)
                )  # Then match it to the hints-side bin
        all_matches = np.array(all_matches)
        assert (
            len(matches) == self.num_mentioned
            and np.sum(all_matches[:, 1] == self.num_mentioned)
            == self.pad_size - self.num_mentioned
        )
...
class Semantic3dPoseReferenceDataset(Dataset):
    def __init__(self, path_numpy, path_scenes, scene_name, pad_size, split=None):
        ...
        # Load objects
        self.scene_objects = pickle.load(
            open(osp.join(self.path_scenes, "train", self.scene_name, "objects.pkl"), "rb")
        )
        self.poses = pickle.load(
            open(osp.join(self.path_scenes, "train", self.scene_name, "poses.pkl"), "rb")
        )
        self.pose_descriptions = pickle.load(
            open(
                osp.join(self.path_scenes, "train", self.scene_name, "pose_descriptions.pkl"), "rb"
            )
        )
        ...
        if split is not None:
            ...
            self.pose_descriptions = [
                p for (i, p) in enumerate(self.pose_descriptions) if indices[i]
            ]
    ...
    def __getitem__(self, idx):
        pose, pose_description, hint_descriptions = (
            self.poses[idx],
            self.pose_descriptions[idx],
            self.hint_descriptions[idx],
        )
        ...
        cell_bbox = np.hstack(
            (np.min(mentioned_bboxes[:, 0:3], axis=0), np.max(mentioned_bboxes[:, 3:6], axis=0))
        )
        ...
        if True:
            ...
            cell_bbox[[0, 1, 3, 4]] += np.random.randint(
                -max_offset_size, +max_offset_size, size=4
            )  # Offset only in x-y-plane
            ...
            for mentioned_id in hint_ids:
                ...
                if len(points_in_cell) / len(obj.points_w) >= 1 / 3:
                    mentioned_object = Object3D.from_mock_data(
                        {
                            "label": obj.label,
                            "id": obj.id,
                            "points_w": obj.points_w,
                            "color": obj.color,
                        }
                    )
        ...
        # CARE: using "mock" Object3Ds here, not cell objects
        for obj in mentioned_objects:
            cell_object = Object3D.from_mock_data(
                {"label": obj.label, "id": obj.id, "points_w": obj.points_w, "color": obj.color}
            )
        ...
        for obj in self.scene_objects:
            ...
            if (
                len(cell_objects) == self.pad_size
            ):  # CARE: removing some distractors if over pad_size, this uses ground-truth data!
                break
        ...
        while len(cell_objects) < self.pad_size:
            ...
            cell_objects.append(
                Object3D.from_mock_data(
                    {"points_w": points_w, "label": label, "color": color, "id": -1}
                )
            )
        ...
        matches, all_matches = np.array(matches), np.array(all_matches)
        assert np.sum(all_matches[:, 1] == len(hint_ids)) == len(cell_objects) - len(
            mentioned_objects
        )
...
class Semantic3dPoseReferenceDatasetMulti(Dataset):
    def __init__(self, path_numpy, path_scenes, scene_names, pad_size, split=None):
        ...
        self.split = split
        self.datasets = [
            Semantic3dPoseReferenceDataset(path_numpy, path_scenes, scene_name, pad_size, split)
...
class Semantic3dObjectReferenceDataset(Dataset):
    def __init__(self, path_numpy, path_scenes, num_distractors="all", split=None):
        ...
        self.list_descriptions = pickle.load(
            open(
                osp.join(
                    self.path_scenes, "train", self.scene_name, "list_object_descriptions.pkl"
                ),
                "rb",
            )
        )
        self.text_descriptions = pickle.load(
            open(
                osp.join(
                    self.path_scenes, "train", self.scene_name, "text_object_descriptions.pkl"
                ),
                "rb",
            )
        )
        self.hint_descriptions = pickle.load(
            open(
                osp.join(
                    self.path_scenes, "train", self.scene_name, "hint_object_descriptions.pkl"
                ),
                "rb",
            )
        )
        assert (
            len(self.list_descriptions)
            == len(self.text_descriptions)
            == len(self.hint_descriptions)
        )
        ...
        if split is not None:
            ...
            self.list_descriptions = [
                ld for (i, ld) in enumerate(self.list_descriptions) if indices[i]
            ]
            self.text_descriptions = [
                td for (i, td) in enumerate(self.text_descriptions) if indices[i]
            ]
            self.hint_descriptions = [
                hd for (i, hd) in enumerate(self.hint_descriptions) if indices[i]
            ]
    ...
    def __getitem__(self, idx):
        ...
        target_idx, target_id, target_class = [
            (i, obj.id, obj.label) for (i, obj) in enumerate(list_descr) if obj.is_target
        ][0]

        mentioned_objects = [
            obj for obj in self.scene_objects if obj.id in mentioned_ids
        ]  # Including target
        ...
        if self.num_distractors == "all":
            ...
            distractor_objects = np.random.choice(
                [obj for obj in self.scene_objects if obj.id not in mentioned_ids],
                size=self.num_distractors,
                replace=False,
            )
        else:
            distractor_objects = []
        ...
        offset_vectors = target_center - np.vstack(
            (mentioned_objects_positions, distractor_objects_positions)
        )

        mentioned_objects_mask = np.zeros(
            len(mentioned_objects_classes) + len(distractor_objects_classes), dtype=np.int64
        )
        ...
        assert len(all_matches) == len(mentioned_objects) + len(distractor_objects) and np.sum(
            all_matches[:, 1] == bin_idx
        ) == len(distractor_objects)

        # Always stacks mentioned then distractors
        return {
            "target_idx": target_idx,
            "objects_classes": mentioned_objects_classes + distractor_objects_classes,
            "objects_positions": np.vstack(
                (mentioned_objects_positions, distractor_objects_positions)
            ),
...
class Semantic3dCellRetrievalDataset(Dataset):
    def __init__(self, path_numpy, path_scenes, scene_name, mention_features, split=None):
        ...
        self.cells = pickle.load(
            open(
                osp.join(
                    self.path_scenes, "train", self.scene_name, "cell_object_descriptions.pkl"
                ),
                "rb",
            )
        )
    ...
    def __getitem__(self, idx):
        ...
        negative_indices = [
            i for i in range(len(self)) if i != idx and len(self.cell_texts) != len(description)
        ]
...
class Semantic3dCellRetrievalDatasetMulti(Dataset):
    def __init__(self, path_numpy, path_scenes, scene_names, mention_features, split=None):
        ...
        self.split = split
        self.datasets = [
            Semantic3dCellRetrievalDataset(
                path_numpy, path_scenes, scene_name, mention_features, split
            )
            for scene_name in scene_names
        ]
...
if __name__ == "__main__":
    dataset1 = Semantic3dPoseReferenceDataset(
        "./data/numpy_merged/",
        "./data/semantic3d",
    ...
    dataset = Semantic3dPoseReferenceMockDataset(
        args, dataset1.get_known_classes(), COLORS_S3D, COLOR_NAMES_S3D
    )
    dataloader = DataLoader(
        dataset, batch_size=2, collate_fn=Semantic3dObjectReferenceMockDataset.collate_fn
    )
    data = dataset[0]
