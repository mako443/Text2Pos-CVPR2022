...
def localize_ransac(clustered_objects, description):
    ...
    for iter in range(1000):
        ...
        anchor0_description_idx = np.random.choice(
            [i for i in range(len(description)) if description[i][0] == "infront"]
        )
        anchor1_description_idx = np.random.choice(
            [i for i in range(len(description)) if description[i][0] == "behind"]
        )
        ...
        anchor0_object_idx = np.random.choice(
            [
                i
                for i in range(len(object_classes))
                if object_classes[i] == anchor0_class and object_colors[i] == anchor0_color
            ]
        )
        anchor1_object_idx = np.random.choice(
            [
                i
                for i in range(len(object_classes))
                if object_classes[i] == anchor1_class and object_colors[i] == anchor1_color
            ]
        )

        center_estimate = (
            1 / 2 * (object_centers[anchor0_object_idx] + object_centers[anchor1_object_idx])
        )
        vector = object_centers[anchor0_object_idx] - object_centers[anchor1_object_idx]
        ori_estimate = np.arctan2(vector[1], vector[0])

        remaining_object_indices = np.array(
            [
                i
                for i in range(len(object_classes))
                if i not in (anchor0_object_idx, anchor1_object_idx)
            ]
        )
        remaining_description_indices = np.array(
            [
                i
                for i in range(len(description))
                if i not in (anchor0_description_idx, anchor1_description_idx)
            ]
        )
        ...
        for idx_verify in remaining_description_indices:
            ...
            angle_differences[
                object_classes != description[idx_verify][1].label
            ] = np.inf  # Remove objects from false classes
            angle_differences[
                object_colors != discretize_color(description[idx_verify][1].color)
            ] = np.inf  # Remove objects from false colors
        ...
        if angle_error < best_error:
            best_error, best_center, best_ori, best_indices = (
                angle_error,
                center_estimate,
                ori_estimate,
                (anchor0_object_idx, anchor1_object_idx, *found_verify_indices),
            )
