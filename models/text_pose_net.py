...
class TextPoseNet(torch.nn.Module):
    def __init__(self,known_words, embedding_dim):
        ...
        self.word_embedding.weight.requires_grad_(False) #Training the embedding or not made only a small difference in the past.

        self.lstm = nn.LSTM(self.embedding_dim, self.embedding_dim)

        #Add ReLU and Linear layer: much stronger ✓
        self.linear=nn.Linear(self.embedding_dim, 6) # 6-D output: 2D position, 4D quaternion rotation
