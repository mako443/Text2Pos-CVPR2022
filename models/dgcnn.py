...
class DGMatch(torch.nn.Module):
    def __init__(self, known_classes, known_words, embed_dim, k, use_layers):
        ...
        self.graph1 = gnn.DynamicEdgeConv(
            get_mlp([4 * embed_dim, 4 * embed_dim, 2 * embed_dim], add_batchnorm=True),
            k=k,
            aggr="max",
        )
        self.graph2 = gnn.DynamicEdgeConv(
            get_mlp([4 * embed_dim, 4 * embed_dim, 2 * embed_dim], add_batchnorm=True),
            k=k,
            aggr="max",
        )
        ...
        # Prediction layers
        self.mlp_object_ref = get_mlp(
            [2 * embed_dim, embed_dim, 1]
        )  # Predict a reference confidence for each obj
        self.mlp_target_class = get_mlp(
            [embed_dim, 16, len(self.known_classes)]
        )  # Predict the class of the referred object based on the text
        self.mlp_object_class = get_mlp(
            [2 * embed_dim, embed_dim, 16, len(self.known_classes)]
        )  # Predict the class of each object based
    ...
    def forward(self, object_classes, object_positions, descriptions):
        ...
        features = features.reshape(
            (-1, 2 * self.embed_dim)
        )  # [B*num_obj, 2*DIM], reshaped to process batch as sparse graph
...
if __name__ == "__main__":
    ...
    object_classes = [
        ["high vegetation", "low vegetation", "buildings", "hard scape", "cars"] for i in range(3)
    ]
