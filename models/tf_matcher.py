...
class TransformerMatch(torch.nn.Module):
    def __init__(self, known_classes, known_words, args):
        ...
        print(f'TransformerMatch: dim: {self.embed_dim}, features: {use_features}, nhead: {args.nhead}, dim_ff: {args.dim_ff}, iters: {args.sinkhorn_iters}')
    ...
    def forward(self, objects, hints, verbose=False):
        ...
        if verbose:
            print('before')
            d = cdist(obj_encodings[0].cpu().detach().numpy(), hint_encodings[0].cpu().detach().numpy())
...
if __name__ == "__main__":
    ...
    model = TransformerMatch(['high vegetation', 'low vegetation', 'buildings', 'hard scape', 'cars'], 'a b c d e'.split(), args)
