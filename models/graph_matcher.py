...
class GraphMatch(torch.nn.Module):
    def __init__(
        ...
        sinkhorn_iters,
        num_layers,
        use_features=["class", "color", "position"],
    ):
        ...
        self.graph1 = gnn.DynamicEdgeConv(
            get_mlp([2 * embed_dim, embed_dim, embed_dim], add_batchnorm=True), k=k, aggr="max"
        )
    ...
    def forward(self, objects, hints):
        ...
        if "class" in self.use_features:
            class_embedding = self.class_embedding(
                torch.tensor(class_indices, dtype=torch.long, device=self.device)
            )
        ...
        if self.num_layers == 0:  # Only MLP for each side
            ...
            hint_encodings = self.mlp_residual(
                torch.cat((hint_encodings1, hint_encodings2), dim=-1)
            )
        ...
        obj_encodings = F.normalize(
            obj_encodings.reshape((batch_size, -1, self.embed_dim)), dim=-1
        )  # [B, num_obj, DIM]
        hint_encodings = F.normalize(
            hint_encodings.reshape((batch_size, -1, self.embed_dim)), dim=-1
        )  # [B, num_hints, DIM]
